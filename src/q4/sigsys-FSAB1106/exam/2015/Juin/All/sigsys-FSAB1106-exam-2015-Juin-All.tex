...
\begin{itemize}
	...
	\item Calculer la réponse impulsionnelle de ce système pour le jeu de paramètre suivants: $L = 5 mH$, $C=100\mu F$ et $R = 15\Omega$
...
1. Représentez graphiquement le résultat d’un DFT de taille 8 obtenue à partir du signal x[n] dans lequel on insère chaque fois un 0 entre deux échantillons successifs ? Expliquez\\
...
\begin{solution}
	\begin{itemize}
		...
		s+K&\ldots&\ldots\\
		s+1&\ldots&\ldots
		\end{bmatrix} *\cfrac{1}{(s+K)((s+4)(s+2)+1)}
		$$
		$$H(s) = \cfrac{s-K+2}{2(s+K)((s+4)(s+2)+1)} $$
	...
	\item $K =1$
	$$H(s) = \cfrac{s+1}{2(s+1)((s+4)(s+2)+1)}= \cfrac{s+1}{2(s+1)(s+3)^2} $$
	...
	\item K = -2
	$$H(s) = \cfrac{s+4}{2(s-2)(s+3)^2}$$
	Nous avons une valeur propre qui est aussi un pole a gauche de l'axe complexe, le système est donc instable (interne et bibo)
